            ...
            <TabsContent value='activities' className='h-[calc(100%-60px)] overflow-y-auto border-black/10 border-solid shadow-none bg-white/60 backdrop-blur-sm border rounded-xl'>
              <ul className='space-y-4 p-4'>
                {activities.map((activity, index) => (
                  <li key={index} className='flex flex-col gap-2 border-b p-2 text-sm'>
                    <div className='flex items-center gap-2'>
                      <span
                        className={`
               ${activity.status === 'complete' ? 'bg-green-500' : activity.status === 'error' ? 'bg-red-500' : 'bg-yellow-500'} min-w-2 min-h-2 h-2 block rounded-full
              `}
                      >
                        &nbsp;
                      </span>
                    ...
                    {activity.timestamp && (
                      <span className='text-xs text-muted-foreground'>{format(activity.timestamp, 'HH:mm:ss')}</span>
                    )}
                  </li>
                ))}
              </ul>
