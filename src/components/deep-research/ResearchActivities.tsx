            ...
            <TabsContent value='activities' className='h-[calc(100%-60px)] overflow-y-auto border-black/10 border-solid shadow-none bg-white/60 backdrop-blur-sm border rounded-xl '>
              <ul className='space-y-[4px] p-[4px]'>
                {activities.map((activity, index) => (
                  <li key={index} className='flex flex-col gap-[2px] border-b p-[2px] text-sm'>
                    <div className='flex items-center gap-[2px]'>
                      <span
                        className={cn(
                          "min-w-[2px] min-h-[2px] h-[2px] block rounded-full",
                          activity.status === "complete" ? "bg-green-500" : activity.status === "error" ? "bg-red-500" : "bg-yellow-500",
                        )}>
                        &nbsp;
                      </span>

                      <p>{activity.message.includes("https://") ? activity.message.split("https://")[0] + activity.message.split("https://")[1].split("/")[0] : activity.message}</p>
                    </div>
                    {activity.timestamp && <span className='text-xs text-muted-foreground'>{format(activity.timestamp, "HH:mm:ss")}</span>}
                  </li>
                ))}
              </ul>
            </TabsContent>
              ...
              <TabsContent value='sources' className='h-[calc(100%-60px)] overflow-y-auto shadow-none bg-white/60 backdrop-blur-sm border rounded-xl border-black/10 border-solid'>
                <ul className='space-y-[4px] p-[4px]'>
                  {sources.map((source, index) => {
                    return (
                      <li key={index} className='flex flex-col gap-[2px] border-b p-[2px]'>
